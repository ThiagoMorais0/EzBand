...
public class BandaServiceImpl implements BandaService {
    ...
    @Override
    public BandaDTO getInfo(Long idBanda) throws ServiceException {
        return new BandaDTO(
                bandaDao.findById(idBanda)
                        .orElseThrow(() -> new ServiceException("Banda não encontrada"))
        );
    }

    @Override
    public void cadastrarUsuario(Long idBanda, Long idUsuario, String instrumentos) {
        musicoBandaService.cadastrarUsuarioEmBanda(Context.getUsuarioLogado(), bandaDao.findById(idBanda).get(), instrumentos);
    }
    ...
    public List<InfoMembroBandaDTO> buscarMembros(Long idBanda) {
        return musicoBandaService.buscarMembrosPorIdBanda(idBanda)
                .stream().map(InfoMembroBandaDTO::new).collect(Collectors.toList());
    }

    @Override
    public void novaBanda(CadastroBandaDTO bandaDTO, MultipartFile logo) {
    ...
    @Override
    public BandaDTO buscarBandaParaIngressar(Long idBanda) throws ServiceException {
    ...
    @Override
    public ShowsFuturosDTO buscarShowsFuturosBanda(Long idBanda, Long idUsuario) throws ServiceException {
        ...
        try{
            ...
            throw new ServiceException("Erro ao buscar shows");
        }

        return showsFuturosDTO;
    }
    @Override
    public EnsaiosFuturosDTO buscarEnsaiosFuturosBanda(Long idBanda, Long idUsuario) throws ServiceException {
        ...
        try {
            ...
            throw new ServiceException("Erro ao buscar ensaios");
        }
    ...
    private List<EnsaioDTO> buscarEnsaiosAguardandoPorBandaEUsuario(Long idBanda, Long idUsuario) {
        return eventoHelperService.buscarEnsaiosAguardandoPorBandaEUsuarioOrdenadoPorData(idBanda, idUsuario).stream().map(EnsaioDTO::new).collect(Collectors.toList());
    }

    private List<EnsaioDTO> buscarEnsaiosPendentesPorBandaEUsuario(Long idBanda, Long idUsuario) {
        return eventoHelperService.buscarEnsaiosPendentesPorBandaEUsuarioOrdenadoPorData(idBanda, idUsuario).stream().map(EnsaioDTO::new).collect(Collectors.toList());
    }

    private List<ShowDTO> buscarShowsAguardandoPorBandaEUsuario(Long idBanda, Long idUsuario) {
        return eventoHelperService.buscarShowsAguardandoPorBandaEUsuarioOrdenadoPorData(idBanda, idUsuario).stream().map(ShowDTO::new).collect(Collectors.toList());
    }

    private List<ShowDTO> buscarShowsPendentesPorBandaEUsuario(Long idBanda, Long idUsuario) {
        return eventoHelperService.buscarShowsPendentesPorBandaEUsuarioOrdenadoPorData(idBanda, idUsuario).stream().map(ShowDTO::new).collect(Collectors.toList());
    }
    ...
    private static void verificarSeBandaPermiteEntradaPorConvite(Banda banda) throws ServiceException {
        if(!banda.getParametros().getPermiteEntradaPorConvite()){
            throw new ServiceException("A banda " + banda.getNome() + " não permite entrada por convite");
        }
    }

    private static void verificarSeUsuarioJaEstaNaBanda(Banda banda) throws ServiceException {
        for(MusicoBanda musicoBanda : banda.getMusicos()){
            if(musicoBanda.getId().getIdUsuario().equals(Context.getUsuarioLogado().getId())){
                throw new ServiceException("Você já está na banda " + banda.getNome());
            }
        }
    }
